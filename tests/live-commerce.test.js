...
test("signed custom requests bind tenant, installation, cart and response freshness",async()=>{
  const secret="x".repeat(43),installation={id:installationId,workspace_id:workspaceId,workspace_domain:"example.com",endpoint:"https://api.example.com/negotiation/v3",credential_ciphertext:seal(secret,`${workspaceId}:${installationId}`,env)},times=[now,now+2000];
  const response={...envelope("context"),asOf:new Date(now+1000).toISOString(),expiresAt:new Date(now+59000).toISOString(),cartFingerprint:fingerprint(cart),currency:"INR",taxBasis:"inclusive",line:{productId:"product-1",variantId:"variant-1",name:"Test",quantity:1,unitPriceMinor:99900,availableToSell:10,approvedFloorMinor:74900,fulfillmentType:"physical"},shipping:{mode:"flat",serviceable:true,merchantCostMinor:5000,customerChargeMinor:5000,rateId:"flat-in"},sales:null,promotions:{codes:[],stackable:false},payment:{method:"prepaid",supported:true,feeMinor:0},checkoutSupported:true};
  const value=await callCustomConnector(installation,"context",{cart},{env,now:()=>times.shift(),resolver:async()=>[{address:"8.8.8.8",family:4}],fetcher:async(_url,request)=>{
