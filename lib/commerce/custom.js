...
export async function validateCustomEndpoint(endpoint,workspaceDomain,{env=process.env,resolver=lookup}={}) {
  ...
  let answers;
  try {answers=await resolver(host,{all:true,verbatim:true});}
  catch {throw Error("CONNECTOR_DNS_UNRESOLVED");}
...
async function readJson(response) {
  ...
  catch {throw Error("CONNECTOR_INVALID_JSON");}
}
...
async function httpError(response) {
  ...
  if(contentType.includes("application/json")) {
    try {
      ...
      const raw=String(value?.code||value?.error||"").toUpperCase().replaceAll(" ","_");
      if(/NONCE.*(TABLE|STORAGE|MIGRATION|UNAVAILABLE)/.test(raw))merchantCode="CONNECTOR_NONCE_STORAGE_UNAVAILABLE";
      else if(/DISABLED/.test(raw))merchantCode="CONNECTOR_DISABLED";
      else if(/MISCONFIG|MISSING.*(ENV|SETTING|SECRET|WORKSPACE|INSTALLATION)/.test(raw))merchantCode="CONNECTOR_MISCONFIGURED";
    } catch(error) {
      if(error.message==="CONNECTOR_RESPONSE_TOO_LARGE")return error.message;
    }
  }
  if(merchantCode)return merchantCode;
  if(response.status===401&&!contentType.includes("application/json"))return "CONNECTOR_HTTP_401_NON_JSON";
  return {401:"CONNECTOR_UNAUTHORIZED",403:"CONNECTOR_FORBIDDEN",409:"CONNECTOR_CONFLICT",422:"CONNECTOR_UNSUPPORTED",503:"CONNECTOR_UNAVAILABLE"}[response.status]||"CONNECTOR_REQUEST_FAILED";
}
...
export async function callCustomConnector(installation,operation,input={},options={}) {
  ...
  if(!response.ok)throw Error(await httpError(response));
  ...
  try {return validateConnectorResponse(await readJson(response),expected,clock());}
  catch(error) {
    if(error.message.startsWith("CONNECTOR_"))throw error;
    if(error.name==="ZodError")throw Error("CONNECTOR_SCHEMA_INVALID");
    throw error;
  }
}
