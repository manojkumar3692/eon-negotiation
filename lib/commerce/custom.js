...
export async function callCustomConnector(installation,operation,input={},options={}) {
  const env=options.env||process.env, fetcher=options.fetcher||fetch, now=options.now?.()??Date.now();
  ...
  const body=JSON.stringify(request),timestamp=String(now),nonce=randomUUID();
  ...
  return validateConnectorResponse(await readJson(response),expected,now);
}
