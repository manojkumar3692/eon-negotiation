import {activeConfiguration,constrainContext,gateSession,logConversion,configuredPrice} from '../conversion/live.js';
...
async function sessionByToken(client,id,token,lock=false,allowAccepted=false) {
  const {rows:[row]}=await client.query(`select s.*,w.domain,i.id as connector_id,i.provider,i.endpoint,i.shop_domain,i.credential_ciphertext,i.refresh_credential_ciphertext,i.token_expires_at,i.scopes,i.config,i.status as installation_status,i.activation_status,i.revoked_at from negotiation.live_sessions s join negotiation.connector_installations i on i.id=s.installation_id join negotiation.workspaces w on w.id=s.workspace_id where s.id=$1 and s.token_hash=$2 ${lock?"for update of s":""}`,[id,hash(token||"")]);
  if(!row||(row.status!=="open"&&!(allowAccepted&&row.status==="accepted"))||(Date.parse(row.expires_at)<=Date.now()&&!(allowAccepted&&row.status==="accepted"))||row.installation_status!=="ready"||row.activation_status==="paused"||row.revoked_at)throw Error("SESSION_UNAVAILABLE");
  return row;
}
...
export async function startLiveSession(input) {
  ...
  const started=await service(async client=>{
    ...
    const rawContext=await providerContext(client,installation,cart,product);
    const gate=await gateSession(client,installation,product,rawContext,input);
    if(!gate.allowed)return {unavailable:true,rateLimited:gate.rateLimited};
    ...
    await client.query("update negotiation.live_sessions set conversion_version=$1,surface=$2,visitor_key=$3,recovery_grant_id=$5 where id=$4",[gate.settings.version,gate.surface,gate.visitorKey,id,gate.recoveryGrantId]);
    await logConversion(client,installation.workspace_id,`started:${id}`,"negotiation_started",id,null,{surface:gate.surface});
    const greeting=gate.settings.config.conversation.welcome;
  ...
  if(started.unavailable)throw Error(started.rateLimited?"RATE_LIMIT":"NEGOTIATION_UNAVAILABLE");
  return started;
}
...
export async function confirmTarget(id,token) {
  return service(async client=>{
    ...
    const settings=await activeConfiguration(client,session.workspace_id);
    if(settings.version!==session.conversion_version)throw Error("CONTEXT_CHANGED");
    const context=constrainContext(await providerContext(client,connectorFromSession(session),session.cart,product),settings.config,product),decision=configuredPrice(context,policy.config,session.pending_target_minor,session.round+1,settings.config,product);
    if(decision.status==="unavailable"){await client.query("update negotiation.live_sessions set pending_target_minor=null,context_snapshot=$1,updated_at=now() where id=$2",[context,id]);const reply=decision.reason==="existing_promotion_better"?"I can’t improve on your verified coupon price with this offer. You can continue with that coupon on the store.":"I can’t create an offer right now because the required store data or approved pricing limits do not allow it.";await addTurn(client,id,"assistant","unavailable",reply,{reason:decision.reason});return {status:"unavailable",reason:decision.reason,message:reply};}
    ...
    await logConversion(client,session.workspace_id,`quote:${quoteId}`,"offer_created",id,quoteId,{concessionType:"price",policyVersion:policy.version});
...
export async function acceptQuote(id,token,{quoteId,idempotencyKey}) {
  ...
  // Commit the intent before external I/O. A lost provider response must not
  // erase the key/snapshot or cause a retry to reprice its own stock reservation.
  const prepared=await service(async client=>{
    const session=await sessionByToken(client,id,token,true,true);
    ...
    if(prior){
      if(prior.request_hash!==requestHash)throw Error("IDEMPOTENCY_MISMATCH");
      if(["created","paid"].includes(prior.status))return {complete:true,result:{status:"created",checkoutUrl:prior.checkout_url,externalId:prior.external_id}};
      if(prior.status==='started'&&prior.response?.preparedQuote&&prior.response?.preparedContext)return {attemptId:prior.id};
      throw Error('SESSION_UNAVAILABLE');
    }
    if(session.status!=="open")throw Error("SESSION_UNAVAILABLE");
    ...
    const settings=await activeConfiguration(client,session.workspace_id);
    if(settings.version!==session.conversion_version)throw Error("CONTEXT_CHANGED");
    const connector=connectorFromSession(session),context=constrainContext(await providerContext(client,connector,session.cart,product),settings.config,product),decision=configuredPrice(context,policy.config,quote.amount_minor,session.round,settings.config,product);
    ...
    const {rows:[spend]}=await client.query("select coalesce(sum(discount_minor),0)::int as total from negotiation.budget_reservations where workspace_id=$1 and created_at>=date_trunc('day',now()) and (status='committed' or (status='reserved' and expires_at>now()))",[session.workspace_id]);
    ...
    const attemptId=randomUUID(),preparedQuote={id:quote.id,cart:session.cart,amountMinor:quote.amount_minor,shippingMinor:quote.shipping_minor,currency:quote.currency,expiresAt:new Date(quote.expires_at).toISOString(),contextRevision:quote.context_revision};
    await client.query("insert into negotiation.checkout_attempts(id,workspace_id,session_id,quote_id,idempotency_key,request_hash,response) values($1,$2,$3,$4,$5,$6,$7)",[attemptId,session.workspace_id,id,quote.id,idempotencyKey,requestHash,{preparedQuote,preparedContext:context}]);
    ...
    await logConversion(client,session.workspace_id,`accepted:${quote.id}`,"offer_accepted",id,quote.id);
    return {attemptId};
  });
  if(prepared.complete)return prepared.result;
  return service(async client=>{
    const session=await sessionByToken(client,id,token,true,true);
    await client.query("select pg_advisory_xact_lock(hashtextextended($1,0))",[session.workspace_id]);
    const {rows:[attempt]}=await client.query('select * from negotiation.checkout_attempts where id=$1 and session_id=$2 for update',[prepared.attemptId,id]);
    if(!attempt)throw Error('SESSION_UNAVAILABLE');
    if(['created','paid'].includes(attempt.status))return {status:'created',checkoutUrl:attempt.checkout_url,externalId:attempt.external_id};
    if(attempt.status!=='started'||!attempt.response?.preparedQuote||!attempt.response?.preparedContext)throw Error('SESSION_UNAVAILABLE');
    const settings=await activeConfiguration(client,session.workspace_id);
    if(settings.version!==session.conversion_version)throw Error('CONTEXT_CHANGED');
    const {preparedQuote,preparedContext}=attempt.response;
    if(Date.parse(preparedQuote.expiresAt)<=Date.now())throw Error('QUOTE_EXPIRED');
    // Store enforces the immutable quote on first creation; exact-key replays
    // recover its already-created checkout before checking changed inventory.
    const checkout=await providerCheckout(client,connectorFromSession(session),preparedQuote,preparedContext,attempt.idempotency_key);
    await client.query("update negotiation.checkout_attempts set status='created',external_id=$1,checkout_url=$2,response=$3,updated_at=now() where id=$4",[checkout.externalId,checkout.checkoutUrl,checkout,attempt.id]);
    await client.query("insert into negotiation.audit_events(workspace_id,actor_user_id,name,detail) values($1,'system:widget','Checkout created',$2)",[session.workspace_id,`Accepted quote ${quoteId} was handed to ${session.provider} checkout.`]);
    await logConversion(client,session.workspace_id,`checkout:${attempt.id}`,"checkout_created",id,quoteId);
