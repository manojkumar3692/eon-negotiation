...
export async function acceptQuote(id,token,{quoteId,idempotencyKey}) {
  ...
  return service(async client=>{
    ...
    if(prior){if(prior.request_hash!==requestHash)throw Error("IDEMPOTENCY_MISMATCH");if(["created","paid"].includes(prior.status))return {status:"created",checkoutUrl:prior.checkout_url,externalId:prior.external_id};}
    ...
    await logConversion(client,session.workspace_id,`checkout:${attemptId}`,"checkout_created",id,quote.id);
