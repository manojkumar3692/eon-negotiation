...
export function cartTestSelection(products, cartProducts = [], selected = '') {
 const eligible = products.filter(p => p.externalVariantId && cartProducts.some(r => r.id === p.id && r.ready));
 const catalog=products.filter(p=>p.externalVariantId);
 const product=catalog.find(p=>p.id===selected)||eligible[0]||catalog[0]||null;
 const readiness=cartProducts.find(r=>r.id===product?.id);
 return {eligible,product,ready:readiness?.ready===true,reason:readiness?.reason||'Enable this product and save its cost and protected minimum in EON Rules before testing.'};
}
