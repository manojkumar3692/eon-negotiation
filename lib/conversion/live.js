...
export function constrainContext(context,config,product){
 ...
 // A fresh connector cost wins, including an explicit zero. For merchant-configured
 // shipping only, an approved per-order expense can fill a missing store cost.
 // Connector/carrier mode intentionally requires a real quote; unknown never means zero.
 const approvedCost=config.shipping.costMinor;
 const canUseApprovedCost=['free','flat','threshold'].includes(config.shipping.mode)&&Number.isSafeInteger(approvedCost)&&approvedCost>=0;
 const shipping={...context.shipping,merchantCostMinor:context.shipping.merchantCostMinor??(canUseApprovedCost?approvedCost:null)};
 const approvedFee=config.paymentFees?.[context.payment.method];
 const payment={...context.payment,feeMinor:context.payment.feeMinor??(Number.isSafeInteger(approvedFee)&&approvedFee>=0?approvedFee:null)};
 return {...context,shipping,payment,line:{...context.line,approvedFloorMinor:Math.max(context.line.approvedFloorMinor??0,rule.floorMinor,unitContributionFloor)}};
}
export async function gateSession(c,installation,product,context,input){
 ...
 // Invitation eligibility is bound to the stored token, not offer-page query hints.
 const hints=invitation?invitation.signals:(input.signals||{});
