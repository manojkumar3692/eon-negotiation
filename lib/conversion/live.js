...
export async function gateSession(c,installation,product,context,input){
 ...
 const arm=assigned<config.experiment.treatmentBps?'treatment':'control';
 const decision=eligibility({...signals,experimentArm:arm},config.triggers);
 await recordEvent(c,{workspaceId:installation.workspace_id,eventKey:`assignment:${settings.version}:${visitorKey}`,type:'experiment_assigned',mode:'live',visitorKey,properties:{reasonCode:arm,policyVersion:settings.version}});
 await recordEvent(c,{workspaceId:installation.workspace_id,eventKey:`eligibility:${randomUUID()}`,type:'eligibility_evaluated',mode:'live',visitorKey,properties:{reasonCode:decision.state,surface}});
