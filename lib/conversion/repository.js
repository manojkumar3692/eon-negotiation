...
export async function conversionAction(user,id,input){return tenant(user,async c=>{
 ...
 if(input.action==='save'){
  ...
  for(const rule of config.products){const {rows:[p]}=await c.query('select price_minor from negotiation.products where workspace_id=$1 and id=$2',[id,rule.id]);if(!p)throw Error('NOT_FOUND');if(rule.targetMinor>p.price_minor||rule.floorMinor>p.price_minor)throw Error('Conversion: Preferred price and minimum cannot exceed the current catalog price.');}
 ...
 if(input.action==='activate'){
  ...
  for(const rule of s.config.products.filter(p=>p.enabled)){const {rows:[p]}=await c.query('select * from negotiation.products where workspace_id=$1 and id=$2',[id,rule.id]);if(!importReady(p))throw Error('Conversion: Confirm selling prices, shipping and promotion information through a new store sync before activation.');}
