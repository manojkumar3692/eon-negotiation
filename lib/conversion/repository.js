...
export async function conversionState(user,id){return tenant(user,async c=>{
 ...
 const {rows:installations}=await c.query('select id,provider,status,activation_status,last_result from negotiation.connector_installations where workspace_id=$1 and revoked_at is null',[id]);
