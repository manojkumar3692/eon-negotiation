...
export function simulateConversion(product,policy,config,input){
 ...
 // The parsed shopper amount is an order total, not a merchant floor.
 // Clamp it to approved economics before passing it through all candidate guards.
 const requested=Number.isSafeInteger(intent.targetMinor)&&intent.targetMinor>=0;
 const economicMinimum=Math.max(rule.floorMinor*quantity+Math.max(0,shippingCost-shipping)+input.paymentFeeMinor,
   rule.costMinor*quantity+shippingCost+input.paymentFeeMinor+config.minContributionMinor-shipping);
 const testTarget=requested?Math.min(product.price_minor,Math.max(
   Math.ceil(Math.min(rule.targetMinor*quantity,Math.max(0,intent.targetMinor-shipping))/quantity),
   Math.ceil(economicMinimum/quantity))):rule.targetMinor;
 const deals=rankDeals(candidates({publicMinor:product.price_minor,targetMinor:testTarget,floorMinor:rule.floorMinor,costMinor:rule.costMinor,stock:signals.stock},
