...
export function approvedReadinessProduct(product,config){
 ...
 if(!rule?.enabled||!Number.isSafeInteger(rule.costMinor)||rule.costMinor<0||!Number.isSafeInteger(rule.floorMinor)||rule.floorMinor<=0)throw Error('Connector: Save an enabled product with approved cost and minimum in EON Rules before testing this cart.');
