...
// Server-only transaction for public session tokens, OAuth callbacks and
// verified webhooks. Callers must establish their own capability boundary.
export async function service(fn) {
  const client=await pool.connect();
  try {
    await client.query("begin");
    const result=await fn(client);
