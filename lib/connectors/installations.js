...
function publicInstallation(row){return {id:row.id,provider:row.provider,status:row.status,shopDomain:row.shop_domain,endpoint:row.provider==="custom"?row.endpoint:null,lastAttemptAt:row.last_attempt_at,lastSuccessAt:row.last_success_at,error:row.last_error,result:row.last_result,shippingMode:row.shipping_mode,config:row.config,activationStatus:row.activation_status,publicKey:row.status==="ready"?row.public_key:null};}

export async function connectorState(user,id){return tenant(user,async client=>{const workspace=await owned(client,id),{rows}=await client.query("select * from negotiation.connector_installations where workspace_id=$1 and revoked_at is null order by created_at",[id]);return {shopifyConfigured:Boolean(process.env.SHOPIFY_CLIENT_ID&&process.env.SHOPIFY_CLIENT_SECRET),workspaceDomain:workspace.domain,domainVerified:workspace.domain_verified,domainVerificationRecord:`eon-negotiation-verification=${workspace.domain_verification_token}`,currency:workspace.currency,installations:rows.map(publicInstallation)};});}

export async function connectorAction(user,id,input){
  const data=inputSchema.parse(input);
  if(data.action==="configure_custom")return tenant(user,async client=>{
    const workspace=await owned(client,id),endpoint=await validateCustomEndpoint(data.endpoint,workspace.domain),{rows:[existing]}=await client.query("select id from negotiation.connector_installations where workspace_id=$1 and connector_kind='custom'",[id]),installId=existing?.id||randomUUID(),setupSecret=randomBytes(32).toString("base64url"),ciphertext=seal(setupSecret,`${id}:${installId}`);
  ...
  return tenant(user,async client=>{
    ...
    if(data.action==="sync"){
      ...
      do {const result=await providerCatalog(client,row,{cursor,limit:100});for(const item of result.items){if(item.currency!==workspace.currency)throw Error("Connector: Store currency does not match this workspace.");await client.query(`insert into negotiation.products(workspace_id,sku,name,price_minor,floor_minor,stock,source,external_product_id,external_variant_id) values($1,$2,$3,$4,$4,$5,$6,$7,$8) on conflict(workspace_id,external_variant_id) where external_variant_id is not null do update set sku=excluded.sku,name=excluded.name,price_minor=excluded.price_minor,floor_minor=least(negotiation.products.floor_minor,excluded.price_minor),stock=excluded.stock,source=excluded.source,external_product_id=excluded.external_product_id,updated_at=now()`,[id,item.sku,item.name,item.priceMinor,item.availableToSell,row.provider,item.productId,item.variantId]);total++;}cursor=result.nextCursor;if(total>=500&&cursor)throw Error("Connector: Catalog exceeds the current 500-product workspace limit.");}while(cursor);
      await client.query("update negotiation.connector_installations set status='read_only',last_success_at=now(),last_error=null where id=$1",[row.id]);await client.query("insert into negotiation.audit_events(workspace_id,actor_user_id,name,detail) values($1,$2,'Catalog synchronized',$3)",[id,user,`${total} product variants synchronized from ${row.provider}. New products remain non-negotiable until their minimum prices are reviewed.`]);return {saved:total};
    }
    if(data.action==="test"){
      await client.query("update negotiation.connector_installations set last_attempt_at=now() where id=$1",[row.id]);
      try {let result;if(data.cart){if(data.cart.currency!==workspace.currency)throw Error("CURRENCY_MISMATCH");const {rows:[product]}=await client.query("select * from negotiation.products where workspace_id=$1 and (external_variant_id=$2 or sku=$2) limit 1",[id,data.cart.lines[0].variantId]);result=await providerContext(client,row,data.cart,product);if(!result.checkoutSupported)throw Error("CHECKOUT_UNSUPPORTED");}else result=await providerCapabilities(client,row);const status=data.cart?"ready":"read_only",{rows:[updated]}=await client.query("update negotiation.connector_installations set status=$1,last_result=$2,last_success_at=now(),last_error=null where id=$3 returning *",[status,result,row.id]);await client.query("insert into negotiation.audit_events(workspace_id,actor_user_id,name,detail) values($1,$2,$3,$4)",[id,user,data.cart?"Live cart verified":"Connector capabilities verified",data.cart?"Fresh price, inventory, economics and checkout capability passed the platform contract.":"The connector responded to a signed capability check."]);return publicInstallation(updated);}catch(error){const detail=/^(CONNECTOR|SHOPIFY|CHECKOUT|CURRENCY)/.test(error.message)?error.message:"CONNECTOR_CHECK_FAILED";const {rows:[updated]}=await client.query("update negotiation.connector_installations set status='failed',last_error=$1 where id=$2 returning *",[detail,row.id]);return publicInstallation(updated);}
    }
  });
}
